...
def load_validation_data():
    ...
    """
    X = np.load(os.path.join(DATA_DIR, 'X_validate.npy'))
    ids = np.load(os.path.join(DATA_DIR, 'ids_validate.npy'))
...
def submission():
    ...
    model_systole.load_weights(os.path.join(W_DIR, 'weights_systole_best.hdf5'))
    model_diastole.load_weights(os.path.join(W_DIR, 'weights_diastole_best.hdf5'))

    # load val losses to use as sigmas for CDF
    with open(os.path.join(W_DIR, 'val_loss.txt'), mode='r') as f:
    ...
    fi = csv.reader(open(os.path.join(DATA_DIR, 'sample_submission_validate.csv')))
